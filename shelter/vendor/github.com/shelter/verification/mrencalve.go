...
func mrenclave_ecreate(ctx *C.EVP_MD_CTX, blob_size uint64, miscselect uint32, xfrm uint64, ssa_frame_size unsafe.Pointer) bool {
	...
	//buf := make([]byte, 64)
	//GoMemSet(unsafe.Pointer(&buf[0]), 0, 64)
	...
	//fmt.Printf("buffer is 0x%x.\n", buffer.Bytes())

	*(*uint32)(ssa_frame_size) = ssasize
	//fmt.Printf("mmrecreate.ssaframesize is %d, encl_size is 0x%x , MRECREATE is 0x%x\n", ssasize, encl_size, MRECREATE)
...
func mrenclave_eextend(ctx *C.EVP_MD_CTX, offset uint64, data unsafe.Pointer) bool {
	...
	for i = 0; i < 0x1000; i = i + 0x100 {
		...
		//fmt.Printf("mrextend offset is 0x%x, i is 0x%x\n", mmreextend.offset, i)

		if ret := mrenclave_update(ctx, unsafe.Pointer(mmreextend)); ret == false {
		...
		if ret := mrenclave_update(ctx, newdata); ret == false {
			fmt.Errorf("mrextend error with data location 0x%x.\n", temp)
			return false
		}
...
func Measure_Encl(path string, mrenclave unsafe.Pointer) bool {
	var flags uint64
	var offset uint64
	...
	sgxtcs.Ssa_offset = temp

	if ret := mrenclave_ecreate(ctx, temp, uint32(miscselect), xfrm, unsafe.Pointer(&ssa_frame_size)); ret == false {
		return false
	}

	for offset = 0; offset < temp; offset = offset + 0x1000 {
		if offset == 0 {
		...
		if ret := mrenclave_eadd(ctx, offset, flags); ret == false {
			return false
		}

		if ret := mrenclave_eextend(ctx, offset, unsafe.Pointer(&buffer[0+offset])); ret == false {
			C.X_EVP_MD_CTX_free(ctx)
			return false
		}
	}
	...
	flags = SGX_SECINFO_REG | SGX_SECINFO_R | SGX_SECINFO_W | SGX_SECINFO_X
	for offset = temp; offset < temp+uint64(ssa_frame_size*0x1000); offset = offset + 0x1000 {
		if ret := mrenclave_eadd(ctx, offset, flags); ret == false {
			return false
		}
		if ret := mrenclave_eextend(ctx, offset, unsafe.Pointer(&data[0])); ret == false {
			C.X_EVP_MD_CTX_free(ctx)
			return false
		}
	}

	if ret := mrenclave_commit(ctx, mrenclave); ret != true {
		C.X_EVP_MD_CTX_free(ctx)
		return false
	}
