 ...
 * @param der_crt_len On entry contains the size of der_crt buffer. On return holds actual size of certificate in bytes.
 */
static void ecdsa_generate_x509
(
    RsaKey* key,
    uint8_t* der_crt,     /* out */
    int* der_crt_len, /* in/out */
...
)
{
    ...
    char* header_begin = memmem((const char*) headers,headers_len,header_tag,strlen(header_tag));
    ...
    char* header_end = memmem(header_begin,headers_len - (header_begin - headers),"\r\n",strlen("\r\n"));
    assert(header_end);
    int unescaped_len;
...
)
{
    ...
    binary_to_base16(cert_data->enc_ppid, sizeof(cert_data->enc_ppid),encrypted_ppid, sizeof(encrypted_ppid));

    char pceid[4 + 1];
    binary_to_base16((uint8_t*)&cert_data->pce_info.pce_id,sizeof(cert_data->pce_info.pce_id),pceid, sizeof(pceid));

    char pcesvn[4 + 1];
    binary_to_base16((uint8_t*)&cert_data->pce_info.pce_isv_svn,sizeof(cert_data->pce_info.pce_isv_svn),pcesvn, sizeof(pcesvn));

    char cpusvn[32 + 1];
    binary_to_base16((uint8_t*)&cert_data->cpu_svn,sizeof(cert_data->cpu_svn),cpusvn, sizeof(cpusvn));
    ...
    snprintf(url, sizeof(url), "%s?encrypted_ppid=%s&cpusvn=%s&pcesvn=%s&pceid=%s",base_url, encrypted_ppid, cpusvn, pcesvn, pceid);
    ...
    char buf[128];
    int rc = snprintf(buf, sizeof(buf), "Ocp-Apim-Subscription-Key: %.32s",opts->subscription_key);
    ...
    parse_response_header_get_pck_cert(curl, header.data, header.len,(char*) evidence->pck_sign_chain,&evidence->pck_sign_chain_len);
...
)
{
    ...
    binary_to_base16((unsigned char*) fmspc, FMSPC_SIZE_BYTES, fmspc_base16, sizeof(fmspc_base16));
    char url[256];
 ...
 * OID and extract it.
 */
static void extract_fmspc_from_pck_cert
(
    char fmspc[6],
    ecdsa_attestation_evidence_t* evidence
)
...
static void ecdsa_get_pck_cert
(
    ecdsa_attestation_evidence_t* evidence,
    const struct ecdsa_ra_tls_options* opts
)
...
static void collect_attestation_evidence
(
    const sgx_report_data_t* report_data,
    const struct ecdsa_ra_tls_options* opts,
    ecdsa_attestation_evidence_t* evidence
...
static void ecdsa_wolfssl_create_key_and_x509
(
    uint8_t* der_key,
    int* der_key_len,
    uint8_t* der_cert,
