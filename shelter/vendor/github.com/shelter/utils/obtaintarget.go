...
var targetkeyword = "skeleton"
var repokeyword = "github.com"
...
func GetSrcCode(srcurl string, branch string, commitid string, localpath string)(bool){
	var i uint
	if ret := CheckGit(); ret != true{
		fmt.Errorf("CheckGit failed.\n")
		return false
	}

	if ret := checkurl(srcurl, repokeyword); ret != true{
	...
	allcmd := bt.String()
	if _, ret := ExecShell(allcmd); ret != true{
		for i = 0; i < 3; i++{
...
func BuildTargetSrc(path string)(bool){
	if ret := CheckPath(path); ret != true{
		return false
	}
