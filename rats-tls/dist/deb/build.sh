...
for BUILD_MODE in "${BUILD_MODE_SPECIAL[@]}"; do
	cd $SCRIPT_DIR
	sed -e 's/Package: rats-tls/Package: rats-tls-'$BUILD_MODE'/g' debian/control.in > debian/control && \
	sed -e 's/cmake -H. -Bbuild/cmake -DRATS_TLS_BUILD_MODE='$BUILD_MODE' -DBUILD_SAMPLES=on -H. -Bbuild/g' debian/rules.in > debian/rules && \
	sed -i "/rats-tls-server/d" debian/rules && \
	sed -i "/override_dh_strip/adh_strip --exclude=rats-tls-server --exclude=rats-tls-client --exclude=sgx_stub_enclave.signed.so --exclude=librats_tls.a --exclude=librats_tls_u.a --exclude=librtls_edl_t.a --exclude=libtls_wrapper*.a --exclude=libcrypto_wrapper*.a --exclude=libattester*.a --exclude=libverifier*.a" deb/debian/rules && \
